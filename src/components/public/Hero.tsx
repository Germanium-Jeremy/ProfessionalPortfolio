...
export function Hero({ profile, socials }: HeroProps) {
  return (
    <section className="mx-auto grid min-h-[48rem] max-w-[90rem] items-center gap-10 px-5 pb-16 pt-32 md:px-10 lg:grid-cols-[1.04fr_.96fr] lg:pb-24 lg:pt-28">
      ...
      >
        <div className="mb-7 flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.22em] text-cyan-200">
        ...
        <h1 className="max-w-3xl text-[clamp(3.25rem,8vw,7.5rem)] font-black leading-[0.88] tracking-[-0.075em] text-white">
          {profile.fullName.split(" ").map((part, index) => (
            <span
              className={
                index === 1
                  ? "hero-outline-name block text-transparent [-webkit-text-stroke:1px_rgb(165_243_252)]"
                  : "block"
              }
              key={`${part}-${index}`}
            >
        ...
        <div className="mt-9 flex flex-wrap gap-3">
          <a
            href="#work"
            className="group inline-flex items-center gap-3 rounded-full bg-cyan-200 px-5 py-3 text-sm font-bold text-[#07111f] transition-transform hover:-translate-y-0.5"
          >
          ...
          {profile.resumeUrl && (
            <a
              ...
              className="inline-flex items-center gap-2 rounded-full border border-white/20 px-5 py-3 text-sm font-semibold text-white transition-colors hover:border-cyan-200 hover:text-cyan-100"
            >
        ...
        <div className="mt-12 flex flex-wrap items-center gap-5 border-t border-white/10 pt-5">
          ...
          {socials.map((social) => (
            <a
              ...
              className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-400 transition-colors hover:text-cyan-200"
            >
              {social.kind}
            </a>
          ))}
      ...
      >
        ...
        <div className="relative aspect-[4/5] overflow-hidden rounded-[2.25rem] border border-white/15 bg-[#0d2139] shadow-2xl shadow-black/40">
          ...
          {profile.avatarUrl ? (
            <Image
              ...
              className="object-contain object-center mix-blend-luminosity opacity-90"
            />
          ) : (
            <div className="absolute inset-0 grid place-items-center text-[12rem] font-black tracking-[-0.15em] text-white/15">
              {profile.fullName.slice(0, 1)}
            </div>
          )}
          <div className="absolute left-6 top-6 flex items-center gap-2 rounded-full border border-white/15 bg-[#07111f]/40 px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-white backdrop-blur">
            <Sparkles className="h-3 w-3 text-cyan-200" /> Portfolio 2026
          </div>
        ...
        <div className="absolute -bottom-5 -left-4 hidden rounded-2xl border border-white/15 bg-[#102440]/90 px-4 py-3 text-xs text-slate-300 shadow-xl backdrop-blur sm:block">
          Design <span className="mx-1 text-cyan-200">×</span> Engineering
        </div>
      </motion.div>
    </section>
  );
