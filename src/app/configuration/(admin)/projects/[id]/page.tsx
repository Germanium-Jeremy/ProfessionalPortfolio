...
export default function ProjectEditor() {
  ...
  const onSubmit = async (data: ProjectInput & { links: ProjectLinkInput[]; facts: ProjectFactInput[]; skillIds: string[] }) => {
    try {
      ...
      if (!result.ok) throw new Error(result.error?.message || 'Failed to save project');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex border-b border-slate-200 dark:border-slate-700">
        {( ['basics', 'description', 'links', 'facts', 'skills', 'gallery'] as Tab[] ).map(tab => (
          <button
            key={tab}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white dark:bg-slate-800 p-6 rounded-lg border border-slate-200 dark:border-slate-700 space-y-6">
        {activeTab === 'basics' && (
        ...
        {activeTab === 'gallery' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              {getValues('gallery')?.map((url: string, index: number) => (
                <div key={index} className="relative aspect-video bg-slate-100 dark:bg-slate-700 rounded-md overflow-hidden border border-slate-200 dark:border-slate-600">
